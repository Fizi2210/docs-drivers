...
Queries are then run within the security context of the given user, and not the one under which the `Driver` was instantiated with (i.e., home database, permissions, etc.). +
...
The parameter `impersonated_user` provides a similar functionality, is available earlier than version 5.14, and works on all server versions.
