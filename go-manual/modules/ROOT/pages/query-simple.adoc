...
The query is then run within the security context of the given user, and not the one under which the driver was instantiated with (i.e., home database, permissions, etc.). +
Session authentication requires a server version >= 5.8.
...
The callback `neo4j.ExecuteQueryWithImpersonatedUser()` provides a similar functionality, is available earlier than version 5.14, and works on all server versions.
However, impersonation requires the user under which the driver was created to have the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/dbms-administration/#access-control-dbms-administration-impersonation[appropriate permissions].
